...
export async function POST(req: NextRequest) {
  ...
  // Cloudflare Workers では process.env は使えないため getCloudflareContext を使用
  let apiKey: string | undefined;
  try {
    const { env } = await getCloudflareContext({ async: true }) as unknown as { env: { OPENAI_API_KEY?: string } };
